...
public class MultithreadedStatTask extends AbstractStatTask {
    ...
    /* 实例初始化块 */ {
        ...
        inputBufferSize = null != strBufferSize ? Integer.parseInt(strBufferSize) : 8192;
        String strBatchSize = System.getProperty("x.batch.size");
        batchSize = null != strBatchSize ? Integer.parseInt(strBatchSize) : 2000;
    }
    ...
    /**
     * 该线程专门负责日志文件的读取并将其读取到的一批日志记录填充到指定的日志记录集（ RecordSet 类的实例，代表一批日志记录，
     * 丽、码见清单 4-8 ）中 。
     * <p>
     * 日志文件读取线程
     */
    protected AbstractLogReader createLogReader() {
        return new LogReaderThread(in, inputBufferSize, batchSize);
    }
}